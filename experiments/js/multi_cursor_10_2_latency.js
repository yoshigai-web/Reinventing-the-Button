...
function draw() {
    ...
    drawButton();
    drawCursor();
    if (btnPressed && millis() - pressedTime > waitTime) {
        if (int(random(10*30)) == 0 && !sound.isPlaying()) sound.play();
    }
    if (btnPressed && millis() - pressedTime > waitTime + 500) {
        init();
    }
}
...
function init() {
    btnPressed = false;
    atari = shuffle(atari);
    waitTime = random(1000);
}
function drawButton() {
    for (let i = 0; i < btnNum; i++) {
        if (btnPressed && atari[0] == i && millis() - pressedTime > waitTime) fill(255, 0, 0);
